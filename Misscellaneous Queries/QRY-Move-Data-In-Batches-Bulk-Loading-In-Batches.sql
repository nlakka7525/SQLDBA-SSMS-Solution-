...
from (select top 1 LogId from [dbo].[TBL_CampaignLinkDetailsLog_old] with (nolock) order by LogId desc) t,
	(select top 1 LogId from [dbo].[TBL_CampaignLinkDetailsLog] with (nolock) order by LogId desc) o
go

select top 1 LogId from [dbo].[TBL_CampaignLinkDetailsLog_Ajay] with (nolock) order by LogId desc
select top 1 LogId from [dbo].[TBL_CampaignLinkDetailsLog] with (nolock) order by LogId desc
go
...
select @LogId_start = min(LogId), @LogId_end = max(LogId) from [dbo].[TBL_CampaignLinkDetailsLog] o;
...
--truncate table KYC_CI.[dbo].[TBL_CampaignLinkDetailsLog_Ajay]
...
SELECT LogDate, ClientCode, ClientName, Age, EmailId, MobileNo, Gender, ProbabilityParameter, B2bOrB2c, CvlKraVerifiedFlag, MarginFundAddedFlag, FnOActiveFlag, FundValue, WHATSAPPONBOARD, FOACT10FREE, ITRADE, FREE_STOCK, WHATSAPPSB, FOCOMMODITY, FOACT, PROFITLOSS, FA1TF, FA4TF, WHATSAPP, FOFIFTY, FOCURRENCY, OnBoardingDate, TradingDay, TradeWin, DIY, REF_EARN, REF_EARN_CODE, REF_EARN_CNT, FA4TF_WA, FREE_ETF, PushedFrom, TradingDay1, TradingDay2, TradingDay3, TradingDay_Flag, IsTraded_Flag, control_flag, Profiling, Voucher_Plan, LogId, Cust_Support, CouponCode, ARQ_REF_EARN_CODE, EncryptedCltcd, B2bOrB2c_Current, control_flag2, BranchCode
   FROM [dbo].[TBL_CampaignLinkDetailsLog] o with (index ([LogId]))
   WHERE o.LogId >= @LogId_counter and o.LogId < (@LogId_counter + @batchSize);'

-- when 0 rows returned, exit the loop
SET IDENTITY_INSERT KYC_CI.[dbo].[TBL_CampaignLinkDetailsLog_Ajay] ON;
WHILE (@LogId_counter <= @LogId_end) 
BEGIN
   
	print 'Insert data from '+cast(@LogId_counter as varchar(20));

   INSERT KYC_CI.[dbo].[TBL_CampaignLinkDetailsLog_Ajay] WITH (TABLOCK)
 ...
 --  if(@loop_counter > 2)
	--break;
END
SET IDENTITY_INSERT KYC_CI.[dbo].[TBL_CampaignLinkDetailsLog_Ajay] OFF;
go

/*
exec sp_rename 'dbo.TBL_CampaignLinkDetailsLog','TBL_CampaignLinkDetailsLog_old';
go
select 'INSERT the DELTA data'
go
exec sp_rename 'dbo.TBL_CampaignLinkDetailsLog_Ajay','TBL_CampaignLinkDetailsLog';
go
*/

--create index LogId on [KYC_CI].[dbo].[TBL_CampaignLinkDetailsLog] (LogId) with (maxdop = 0);
--go
