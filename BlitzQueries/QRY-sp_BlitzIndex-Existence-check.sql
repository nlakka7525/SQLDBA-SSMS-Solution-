...
--select distinct finding from DBA_Admin..BlitzIndex_Summary_Aug26
--select * from DBA_Admin..BlitzIndex_Detailed_Aug26
...
select	[rename-tsql] = 'EXEC '+TABLE_CATALOG+'..sp_rename '''+(TABLE_SCHEMA+'.'+TABLE_NAME+'.'+QUOTENAME(COLUMN_NAME))+''', '''+REPLACE(LOWER(C.COLUMN_NAME),' ','_')+''', ''COLUMN'';',
		...
		*
from DBA_Admin.INFORMATION_SCHEMA.COLUMNS c
